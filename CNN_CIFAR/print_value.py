...
def plot(optimizers=None, curve_type='train'):
    ...
    for optim in optimizers:
        ...
        # print(accuracy[:4])
        accuracy_best = np.max(accuracy)
        accuracies.append(accuracy_best)
        if count % 5==4:
        # if count % 1==0:
            print(optim+': {:.2f}+-{:.2f}'.format(np.mean(accuracies), np.std(accuracies)))
            accuracies=[]
    ...
    # plt.legend()
    # # plt.show()
    # plt.savefig('{}_curve_res_cifar10_multi.pdf'.format(curve_type), bbox_inches='tight')
