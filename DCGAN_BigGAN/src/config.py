...
class Configurations(object):
    ...
    def load_base_cfgs(self):
        ...
        # epsilon for adaptive gradient methods
        self.OPTIMIZATION.eps = 1e-16
        # final_lr and gamma (for AdaBound)
        self.OPTIMIZATION.final_lr = 0.1
        self.OPTIMIZATION.gamma = 1e-3
        self.OPTIMIZATION.rectify = True
    ...
    def define_optimizer(self, Gen, Dis):
        if self.OPTIMIZATION.type_ == "SGD":
        ...
        elif self.OPTIMIZATION.type_ == "AdaM3":
            ...
            self.OPTIMIZATION.g_optimizer = AdaM3(params=filter(lambda p: p.requires_grad, Gen.parameters()),
                                                             lr=self.OPTIMIZATION.g_lr,
                                                             betas=betas_g,
                                                             weight_decay=self.OPTIMIZATION.g_weight_decay,
                                                             eps=self.OPTIMIZATION.eps,
                                                             rectify=self.OPTIMIZATION.rectify)
            self.OPTIMIZATION.d_optimizer = AdaM3(params=filter(lambda p: p.requires_grad, Dis.parameters()),
                                                             lr=self.OPTIMIZATION.d_lr,
                                                             betas=betas_d,
                                                             weight_decay=self.OPTIMIZATION.d_weight_decay,
                                                             eps=self.OPTIMIZATION.eps,
                                                             rectify=self.OPTIMIZATION.rectify)
        elif self.OPTIMIZATION.type_ == "AdaBelief":
            ...
            self.OPTIMIZATION.g_optimizer = AdaBelief(params=filter(lambda p: p.requires_grad, Gen.parameters()),
                                                             lr=self.OPTIMIZATION.g_lr,
                                                             betas=betas_g,
                                                             weight_decay=self.OPTIMIZATION.g_weight_decay,
                                                             eps=self.OPTIMIZATION.eps,
                                                             rectify=self.OPTIMIZATION.rectify)
            self.OPTIMIZATION.d_optimizer = AdaBelief(params=filter(lambda p: p.requires_grad, Dis.parameters()),
                                                             lr=self.OPTIMIZATION.d_lr,
                                                             betas=betas_d,
                                                             weight_decay=self.OPTIMIZATION.d_weight_decay,
                                                             eps=self.OPTIMIZATION.eps,
                                                             rectify=self.OPTIMIZATION.rectify)
        elif self.OPTIMIZATION.type_ == "AdaBound":
            ...
            self.OPTIMIZATION.g_optimizer = AdaBound(params=filter(lambda p: p.requires_grad, Gen.parameters()),
                                                             lr=self.OPTIMIZATION.g_lr,
                                                             betas=betas_g,
                                                             weight_decay=self.OPTIMIZATION.g_weight_decay,
                                                             eps=self.OPTIMIZATION.eps,
                                                             final_lr=self.OPTIMIZATION.final_lr, 
                                                             gamma=self.OPTIMIZATION.gamma)
            self.OPTIMIZATION.d_optimizer = AdaBound(params=filter(lambda p: p.requires_grad, Dis.parameters()),
                                                             lr=self.OPTIMIZATION.d_lr,
                                                             betas=betas_d,
                                                             weight_decay=self.OPTIMIZATION.d_weight_decay,
                                                             eps=self.OPTIMIZATION.eps,
                                                             final_lr=self.OPTIMIZATION.final_lr, 
                                                             gamma=self.OPTIMIZATION.gamma)
        elif self.OPTIMIZATION.type_ == "Yogi":
            ...
            self.OPTIMIZATION.g_optimizer = Yogi(params=filter(lambda p: p.requires_grad, Gen.parameters()),
                                                             lr=self.OPTIMIZATION.g_lr,
                                                             betas=betas_g,
                                                             weight_decay=self.OPTIMIZATION.g_weight_decay,
                                                             eps=self.OPTIMIZATION.eps)
            self.OPTIMIZATION.d_optimizer = Yogi(params=filter(lambda p: p.requires_grad, Dis.parameters()),
                                                             lr=self.OPTIMIZATION.d_lr,
                                                             betas=betas_d,
                                                             weight_decay=self.OPTIMIZATION.d_weight_decay,
                                                             eps=self.OPTIMIZATION.eps)
            ...
            self.OPTIMIZATION.g_optimizer = RAdam(params=filter(lambda p: p.requires_grad, Gen.parameters()),
                                                             lr=self.OPTIMIZATION.g_lr,
                                                             betas=betas_g,
                                                             weight_decay=self.OPTIMIZATION.g_weight_decay,
                                                             eps=self.OPTIMIZATION.eps)
            self.OPTIMIZATION.d_optimizer = RAdam(params=filter(lambda p: p.requires_grad, Dis.parameters()),
                                                             lr=self.OPTIMIZATION.d_lr,
                                                             betas=betas_d,
                                                             weight_decay=self.OPTIMIZATION.d_weight_decay,
                                                             eps=self.OPTIMIZATION.eps)
